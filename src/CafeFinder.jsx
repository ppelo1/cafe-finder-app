...
function KakaoLogo({ size = 18 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" aria-hidden="true">
      <path fill="#191600" d="M12 3C6.48 3 2 6.52 2 10.86c0 2.8 1.9 5.25 4.74 6.62-.16.57-.86 3.06-.9 3.28 0 0-.05.2.07.28.13.09.28.03.28.03.36-.05 3.7-2.42 4.3-2.85.46.06.94.1 1.44.1 5.52 0 10-3.52 10-7.86S17.52 3 12 3z"/>
    </svg>
  );
}
    ...
    <div style={styles.modalOverlay} onClick={onClose}>
      <div style={styles.loginModal} onClick={(event) => event.stopPropagation()}>
        ...
        {(isSupabaseConfigured || devLoginEnabled) ? (
          <>
            <button type="button" style={{ ...styles.snsBtn, ...styles.kakaoBtn, ...(busy ? styles.snsBtnBusy : {}) }} disabled={!!busy} onClick={() => handle("kakao")}>
              <span style={styles.snsIcon}><KakaoLogo /></span>
              <span>{busy === "kakao" ? "이동 중…" : "카카오로 계속하기"}</span>
            </button>
            <button type="button" style={{ ...styles.snsBtn, ...styles.googleBtn, ...(busy ? styles.snsBtnBusy : {}) }} disabled={!!busy} onClick={() => handle("google")}>
              <span style={styles.snsIcon}><GoogleLogo /></span>
              <span>{busy === "google" ? "이동 중…" : "Google로 계속하기"}</span>
            </button>
...
function CafeDetailModal({ cafe, onClose, onAddReview, isFavorite, cafeMemo = "", isLoggedIn, onToggleFavorite, onUpdateMemo, onRequireLogin, onEdit }) {
  ...
  const naverMapUrl = `https://map.naver.com/p/search/${encodeURIComponent(cafe.address)}`;
...
const styles = {
  ...
  detailTab: { flex: 1, minHeight: 44, padding: "0 8px", border: "none", borderBottom: "3px solid transparent", background: "transparent", color: COLOR.inkSoft, fontSize: 13, fontWeight: 600, cursor: "pointer", touchAction: "manipulation" },
  detailTabActive: { borderBottomColor: COLOR.accent, color: COLOR.ink },
  ...
  snsBtn: { position: "relative", display: "flex", alignItems: "center", justifyContent: "center", width: "100%", minHeight: 50, marginTop: 10, border: "none", borderRadius: 12, fontSize: 15, fontWeight: 700, letterSpacing: "-0.01em", cursor: "pointer", boxShadow: "0 1px 3px rgba(38,36,31,0.14)" },
  snsBtnBusy: { opacity: 0.6, cursor: "default" },
  snsIcon: { position: "absolute", left: 16, top: "50%", transform: "translateY(-50%)", display: "flex", alignItems: "center", justifyContent: "center", width: 18, height: 18 },
  kakaoBtn: { background: "#FEE500", color: "rgba(0,0,0,0.88)" },
  googleBtn: { background: "#FFFFFF", color: "#1F1F1F", border: "1px solid #DADCE0", boxShadow: "0 1px 2px rgba(60,64,67,0.15)" },
